...
const DASHBOARD_TABS = ["overview", "users", "tools", "vin", "geo", "ltl", "gaps"] as const
...
const STATIC_DATA_GAPS: DashboardResponse["dataGaps"] = [
  {
...
function normalizeTabParam(value: string | null): DashboardTab {
  if (!value) return "overview"
  return DASHBOARD_TAB_SET.has(value as DashboardTab) ? (value as DashboardTab) : "overview"
}
...
export async function buildRs2DashboardResponse(params: DashboardParams): Promise<DashboardResponse> {
  ...
  const availableRow = await dbQuery<{ available_start: string | null; available_end: string | null }>(
    `
      SELECT
  ...
  const response: DashboardResponse = {
    ...
    stateInspections: [],
    geoStateMeta: {
      states_with_inspections: 0,
      zip5_with_lookup: 0,
      points_total_zip5: 0,
      missing_geo_lookup: false,
    },
    fourWeekLift: { status: "skipped" },
    dataGaps: STATIC_DATA_GAPS,
    quality: {
      generated_at_utc: new Date().toISOString(),
      mapping_fallback_usb937: null,
      ambiguous_mapping_keys: 0,
      top_unmapped_usb: [],
    },
  }

  if (params.tab === "overview") {
    const [overviewRow, priorRow, dailyTrendRows, fourWeekLift] = await Promise.all([
    ...
    response.meta.has_prior_data = hasPriorData
    response.overview = {
      scans,
      unique_users: toInt(overview?.unique_users),
      unique_vins: toInt(overview?.unique_vins),
      prior_scans: hasPriorData ? priorScans : null,
      scan_lift_pct_vs_prior: hasPriorData ? ((scans - priorScans) / priorScans) * 100 : null,
    }
    response.dailyTrend = dailyTrendRows.map((row) => ({
      date_pt: row.date_pt,
      scans: toInt(row.scans),
      unique_users: toInt(row.unique_users),
      unique_vins: toInt(row.unique_vins),
    }))
    response.fourWeekLift = fourWeekLift
    return response
  }

  if (params.tab === "users") {
    const topUsersRows = await dbQuery<{ user_id: string; user_email: string; same_day_reports: string; inspections: string }>(
      `
        WITH scoped AS (
    ...
    response.topUsers = topUsersRows.map((row) => ({
      user_id: row.user_id,
  ...
  if (params.tab === "tools") {
    const topToolsRows = await dbQuery<{ tool_name: string; inspections: string; abs_parts: string; srs_parts: string; cel_parts: string }>(
      `
        SELECT
          s.tool_name,
    ...
    response.topTools = topToolsRows.map((row) => ({
  ...
  if (params.tab === "vin") {
    const [topVinsRows, topAccountVinsRows] = await Promise.all([
    ...
    response.topVins = topVinsRows.map((row) => ({
      vin: row.vin,
    ...
    response.topAccountVins = topAccountVinsRows.map((row) => ({
  ...
  if (params.tab === "geo") {
    ...
    response.stateInspections = Array.from(stateTotals.entries())
    ...
    response.geoStateMeta = {
      states_with_inspections: response.stateInspections.length,
  ...
  if (params.tab === "ltl") {
    const [ltlRetailersRows, ltlClickedRows] = await Promise.all([
    ...
    response.ltlRetailers = ltlRetailersRows.map((row) => ({
      buy_from: row.buy_from,
      inspections: toInt(row.inspections),
      total_buy_clicks: toInt(row.total_buy_clicks),
    }))
    response.ltlClickedAccounts = ltlClickedRows.map((row) => ({
      account_id: row.account_id,
      user_email: row.user_email,
      inspections: toInt(row.inspections),
      total_buy_clicks: toInt(row.total_buy_clicks),
  ...
  if (params.tab === "gaps") {
    const [qualityRow, unmappedRows] = await Promise.all([
    ...
    response.quality = {
      generated_at_utc: quality?.generated_at_utc ?? new Date().toISOString(),
...
export function getNormalizedParams(searchParams: URLSearchParams): DashboardParams {
  ...
  if (start <= end) return { start, end, topN, tab }
  return { start: end, end: start, topN, tab }
}
